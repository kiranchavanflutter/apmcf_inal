...
class _BillGeneratePageState extends State<BillGeneratePage> {
            ...
            TextButton(
              onPressed: () {
                ...
                Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => const TraderPage(),
                    ));
              },
              child: const Text('OK'),
            ),
  ...
  void _saveBill() async {
    // Logic to save the bill
    FarmerBillDataBase obj = FarmerBillDataBase(
      product: productController.text,
      rate: num.tryParse(rateController.text) ?? 0.0,
      quantity: quantityController.text,
      date: dateController.text,
      expenses: double.tryParse(expensesController.text) ?? 0.0,
      farmerName: farmerNameController.text,
      place: placeController.text,
      totalBill: double.tryParse(billController.text) ?? 0.0,
      traderName: traderNameController.text,
    );
    await insertData(obj);

    _showDialog('Bill genrate successfully!');
  }
